...
4. generateImageFromPrompt({ finalPrompt, originalImageUrl })     ← backend only
      originalImageUrl 있음 → OpenAI Images edits API (reference image, gpt-image-2는 자동 high fidelity)
...
| 구분 | 설명 |
|------|------|
| Provider | OpenAI Images API (`gpt-image-2`, 기본) |
| `originalImageUrl` **있음** | `POST /v1/images/edits` — reference image를 multipart로 전달 (gen2는 자동 high fidelity) |
| `originalImageUrl` **없음** | `POST /v1/images/generations` — **text-to-image only** |
| 품질·크기 | `quality=medium`, `size=1024x1024` (env로 변경 가능) |
| 타임아웃 | 120초 (gen2 복잡 프롬프트 대응) |
