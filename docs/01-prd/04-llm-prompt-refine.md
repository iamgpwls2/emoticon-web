...
| 언어 | 사용자 입력과 **가능한 한 동일 언어**(한국어 입력 → 한국어 storyPrompt) |
...
> 캐릭터가 기쁜 마음으로 손을 흔들며 「안녕!」이라고 인사하는 이모티콘 장면.
...
| 참조 이미지 | `originalImageUrl`이 있으면 **캐릭터 외형 일관성** 유지 지시 포함 |
| 언어 | storyPrompt와 동일 — 입력 언어 우선 |
