...
2. **환경 변수 파일 만들기**
   - 템플릿: [`frontend/.env.example`](./frontend/.env.example), [`backend/.env.example`](./backend/.env.example)
   - 전체 정책: [`docs/04-security/api-key-policy.md`](./docs/04-security/api-key-policy.md)

   ```bash
   cp frontend/.env.example frontend/.env   # publishable key만
   cp backend/.env.example backend/.env     # SUPABASE_SECRET_KEY 포함
   # Dashboard 값으로 placeholder 교체
   ```
...
전체 목록과 주석은 [`frontend/.env.example`](./frontend/.env.example), [`backend/.env.example`](./backend/.env.example), [`docs/04-security/api-key-policy.md`](./docs/04-security/api-key-policy.md)를 참고하세요.
